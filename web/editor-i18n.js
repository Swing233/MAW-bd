(function initMaweI18n(global) {
  ...
  const EN_TEXT = {
    ...
    '修改字幕': 'Edit subtitle', '字幕文字': 'Subtitle text', '保存修改': 'Save changes', '字幕已修改': 'Subtitle updated',
    'Shift+点击': 'Shift+click', 'Ctrl+点击': 'Ctrl+click',
    '拖拽空白处': 'Drag blank area', '框选字幕（Shift 追加）': 'Box-select subtitles (Shift adds)',
  ...
  const EN_ATTR = {
    ...
    '选择工具（V，默认）：点击选中、拖动移动、拖动边界调整；空白拖拽框选，Shift 追加多选，Alt 临时反转相邻字幕联动，Alt+点击切换禁用': 'Select tool (V, default): click to select, drag to move, drag edges to trim; drag blank area to box-select, hold Shift to add, Alt temporarily reverses adjacent-cue linking, Alt+click toggles disabled',
