...
- 波形字幕块编辑小窗按回车保存，粘贴的换行合并为单行字幕
- 波形时间线按 B 使用播放头的绝对时间分割字幕，不受鼠标或文字光标位置影响
- DeepSeek 校对和 GPT SRT 提示词默认以阿拉伯数字表示明确数字，保留成语和专名原文
- 新版发布后可在应用内一键下载、校验 SHA-256 与应用签名、自动替换并重启；安装失败保留旧版
- 本地 ASR 子进程不再向 `.app` 内写入 `__pycache__`，避免运行后破坏代码签名
- 修复打包版编辑器服务吞掉显式 `--port` 参数的问题
